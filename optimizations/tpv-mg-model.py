...
for it in range(start_iteration, num_cycles):
    ...
    # batch_fom_wrt_perm = torch.zeros((config['num_images'], 100))
    ...
    for image_index, image in enumerate(generated_images):
        ...
        for i, wavelength in enumerate(wavelengths):
            ...
            for z in grating_z_space:
                ...
                forward_e_fields_per_wavelength[i][zc] = np.array(E[0])[1::2]
                zc += 1
        ...
        fom_wrt_perm_per_wavelength_p2 = np.einsum(
            'ijkl,ijkl->ijk', forward_e_fields_per_wavelength, forward_e_fields_per_wavelength)
        ...
        print(f'Binarization: {torch.mean(1-2*torch.abs(image - 0.5))}')
