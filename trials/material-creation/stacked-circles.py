...
for i in range(5):
    ...
    thicknesses, radii = initialization_params[:layers], initialization_params[layers:]
    np.sort(radii)
    print(f'Thicknesses, radii: {thicknesses}, {radii}')
    for i_wavelength, wavelength in enumerate(tqdm(wavelengths, desc="Processing wavelengths", leave=False)):
        ...
        S.SetRegionCircle(Layer = 'Grid0', Material = f'W', Center = (.3, .3), Radius = radii[0] * .3)
        ...
        S.AddLayer(Name = f'Grid2', Thickness = thicknesses[2], Material = 'Vacuum')
        S.SetRegionCircle(Layer = 'Grid2', Material = f'W', Center = (.3, .3), Radius = radii[2] * .3)
    ...
    torch.save(transmitted_power_per_wavelength, f'{ff.home_directory()}/untracked-figures/grid_search/stacked-circles_{thicknesses[0]}_{radii[0]}_backward.pt')
    torch.save(forward_power_per_wavelength, f'{ff.home_directory()}/untracked-figures/grid_search/stacked-circles_{thicknesses[0]}_{radii[0]}_forward.pt')