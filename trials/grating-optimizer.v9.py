...
def gradient_per_image(grating: torch.Tensor, L: float, ang_pol: float):
    ...
    depth = 0.7
    z_buff = 0.15
    z_space = np.linspace(0, 0.5 + depth + 1, 40)
    ...
    for i_wl, wl in enumerate(wavelengths[p:p+2]):
        ...
        S.SetMaterial(Name='AlN', Epsilon=(ff.aln_n[i_wl+p+130]**2-1)*grating[].item()+1)
        ...
        #     eps = grating[ns].item() * (ff.aln_n[i_wl+p+130]**2 - 1) + 1
        ...
        dflux[i_wl] = term.mean(dim=0).squeeze() * (ff.aln_n[i_wl+p+130]**2 - 1)
        del S_adj

    dfom = dflux[0]# - dflux[1]
    dfom = dfom.reshape(-1, x_density).mean(dim=1)
    diff = float(power[0])# - power[1])
    print(f'G: {grating}, D: {dfom}')
    return dfom, diff
