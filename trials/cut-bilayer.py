'''
This file simulates what happens when we make a singular cut in Declan's bilayer film inside the AlN layer, according to the specifications suggested by ./gradient-descent-bilayer.py.
'''
...
config = {
    ...
    "n_incidence_harmonics": int(20),
    "n_grating_harmonics": int(20), # >= config['n_incidence_harmonics'], this tells you how many you need to set
    "n_grating_elements": int(20),
    "ang_polarization": float(90), # Better angle for optimization (generally good at showing what can happen in two dimensions with unpolarized metamaterials)
...
for epoch in range(n_epochs):
    ...
    for i_wl, wl in enumerate(tqdm(wavelengths, desc="Processing wavelengths", leave = False)):
        if wl.item() in exclude_wavelengths or i_wl % 10 != 0:
            continue
        ...
        for ns in range(2):
            ...
            # S.SetRegionRectangle(Layer = 'Grating', Material = f'sq{ns+1}', Center = ((ns+1)/grating.shape[-1] - 1/(2*grating.shape[-1]), .5), Halfwidths = (1/(2*grating.shape[-1]), .5), Angle = 0)
            S.SetRegionRectangle(Layer = 'Grating', Material = f'Vac', Center = ((ns+1)/grating.shape[-1] - 1/(2*grating.shape[-1]), .5), Halfwidths = (1/(2*grating.shape[-1]), .5), Angle = 0)
