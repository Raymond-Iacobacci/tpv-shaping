...
x_measurement_space = make_measurement_grid(L = 1.0, n_cells = x_density, k = config['n_grating_elements'])
z_space = np.linspace(0, .5 + 1.6 + .473 + 1, 30)
z_measurement_space = z_space[(z_space >= .5) & (z_space <= .5 + 1.6)] # Takes only the relevant items
grating = torch.rand((config['n_gratings'], config['n_grating_elements']))
