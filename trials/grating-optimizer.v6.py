...
def gradient_per_image(grating: torch.Tensor, L: float, ang_pol: float, index: int) -> torch.Tensor:
    ...
    for i_wl, wl in enumerate(wavelengths[p:p+2]):
        ...
        S.SetMaterial(Name = 'W', Epsilon = ff.w_n[i_wl+130+p]**2) # This is extraordinarily necessary
        ...
        for ns in range(grating.shape[-1]):
            S.SetMaterial(Name = f'sq{ns+1}', Epsilon = grating[ns].item() * (ff.aln_n[i_wl+130+p]**2-1)+1)
        ...
        dflux_deps[i_wl] = torch.mean(dflux_deps_wl, dim = 0).squeeze() * (ff.aln_n[i_wl+130+p]**2 - 1)
        del S_adj

    dflux_deps = torch.tensor(dflux_deps, requires_grad = True) # Add in the gradient to start the backpropagation step
    dflux_deps = dflux_deps.reshape(-1, x_density).mean(axis=1).detach()

    dfom_deps = dflux_deps[0] - dflux_deps[1]
    print(transmitted_power)
    print(dfom_deps)
    print(transmitted_power[0] - transmitted_power[1])
