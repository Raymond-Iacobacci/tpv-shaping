...
# print(ff.aln_n[0]**2)
# sys.exit(1)
for aln_depth in np.linspace(0.1, 0.8, 140):
    ...
    # with assign_variables(i_wavelength=10, wavelength=.360) as (i_wavelength, wavelength):
        ...
        S.AddLayer(Name = 'Substrate', Thickness = aln_depth, Material = 'AlN')
        ...
        if i_wavelength == 0 and (plotx or ploty or plotz):
            zspace = np.linspace(0, .5+1.6+1, 100)
...
# sys.exit(1)
    ...
    for bandgap in bandgaps:
        ...
        print(f"Substrate depth: {aln_depth}, Bandgap: {bandgap}, FOM: {FOMs[-1]}")
