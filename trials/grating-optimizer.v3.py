...
def gradient_per_image(grating: torch.Tensor, L: float, ang_pol: float, index: int) -> torch.Tensor:
    ...
    for i_wl, wl in enumerate(tqdm(wavelengths, desc="Processing wavelengths", leave = False)):
        ...
        for ns in range(grating.shape[-1]):
            S.SetMaterial(Name = f'sq{ns+1}', Epsilon = grating[ns].item() * (ff.mgo_n[i_wl]**2-1)+1)
        ...
        dflux_deps[i_wl] = torch.mean(dflux_deps_wl, dim = 0).squeeze() * (ff.mgo_n[i_wl]**2 - 1)
        del S_adj
    ...
    # assume `jump`, `wavelengths`, `dfom_dflux` are already defined
    x = wavelengths.detach().numpy()[::jump] * 1000    # e.g. nm
    y = dfom_dflux.detach().numpy()[::jump]

    # build the binary flag: 1 if gradient>0, else 0
    flag = ((y > 0)).astype(int)
    idx100 = np.argmin(np.abs(x - 1000))
    flag[idx100] = 1

    fig, ax1 = plt.subplots(figsize=(8,4))

    # --- left axis: the original gradient scatter ---
    ax1.scatter(x, y, s=5, marker='o', color='tab:blue', label='Gradient')
    ax1.set_xlabel('Wavelength (nm)', fontsize=15)
    ax1.set_ylabel(r'Gradient $\partial C_\text{eff}/\partial\epsilon$',    fontsize=15, color='tab:blue')
    ax1.tick_params(axis='x', labelsize=8)
    ax1.tick_params(axis='y', labelsize=8, labelcolor='tab:blue')

    # --- right axis: the 0/1 flag ---
    ax2 = ax1.twinx()
    ax2.step(x, flag, where='mid', color='tab:red', linewidth=1, label='Emissivity')
    ax2.set_ylabel('Target emissivity', fontsize=15, color='tab:red')
    ax2.tick_params(axis='y', labelsize=8, labelcolor='tab:red')
    ...
    plt.title(r"Sampled $\partial C_\text{eff}/\partial\epsilon$ (every 10th bin)", fontsize=20)
    plt.tight_layout()
    plt.show()
...
if not config['optimizer']:
    ...
    gratings[0].data[20:].fill_(-15.0)
    gratings[0].data[:20].fill_(15.0)
    # gratings[0].data[:].fill_(15)
    # gratings[0].data[:1].fill_(-15.0)
    # gratings[0].data[one_indices].fill_(5)
    # gratings[0].data[zero_indices].fill_(-5)

    for epoch in range(start_epoch, n_epochs):
        gradient = torch.zeros(size = (gratings[0].shape[-1],))
        for i in range(n_gratings):
            sig = torch.sigmoid(gratings[i])
            # sig = (sig * 1.05).clamp(0.0, 1.0)
            dfom_dsig = gradient_per_image(sig, L, config['ang_polarization'], i)
            gradient += dfom_dsig * sig * (1 - sig)
            # gradient += gradient_per_image(gratings[i], L, config['ang_polarization'], i)
            # print(f"Gradient: {gradient}")
        gradient /= n_gratings
        for i in range(n_gratings):
            gratings[i].data -= gradient * config.get('learning_rate', 1e-3)
            # with torch.no_grad():
            #     gratings[i].data.clamp_(-8, 8)
        # with torch.no_grad():
