...
exclude_wavelengths = torch.tensor([.22, .5, 1])
...
depths1=[.7,.722,.75][1:]
widths=[.4,.417,.44][2:3]
periods = [0.9, 1.1, 1.3][1:2]
def gradient_per_image(width, depth1, period):
    print(width, depth1)
    ...
    for i_wl, wl in enumerate(tqdm(wavelengths, desc="Processing wavelengths", leave = False)):
        ...
        S = S4.New(Lattice = ((period, 0), (0, period)), NumBasis = config['n_grating_harmonics'])
        ...
        S.SetRegionRectangle(Layer = 'Grating', Material = 'AlN', Center = (width/2, width/2), Halfwidths = (width/2, width/2), Angle = 0)

        # S.AddLayer(Name = 'Ab1', Thickness = depth2, Material = 'W')
        S.AddLayer(Name = 'Ab2', Thickness = 1, Material = 'W')

        # S.SetRegionRectangle(Layer = 'Ab1', Material = 'Vac', Center = (width/2, (period+width)/2), Halfwidths = (width/2,(period-width)/2), Angle = 0)
        # S.SetRegionRectangle(Layer = 'Ab1', Material = 'Vac', Center = ((period+width)/2, width/2), Halfwidths = ((period-width)/2,width/2), Angle = 0)
        # S.SetRegionRectangle(Layer = 'Ab1', Material = 'Vac', Center = ((period+width)/2, (period+width)/2), Halfwidths = ((period-width)/2, (period-width)/2), Angle = 0)
...
# emissivity, sample_mask = gradient_per_image(0, 0)
# if config['logging']:
#     torch.save(emissivity, f"{1.1}_{.473}_cut_emission.pt")
# idx = sample_mask.nonzero(as_tuple=True)[0]
# fom = ff.power_ratio(wavelengths[idx], emissivity[idx], ff.T_e, .726)
# print(fom.detach())
# if plotting:
#     plt.plot(emissivity.detach())
#     plt.show()


for width in widths:
    for depth1 in depths1:
        for period in periods:

            emissivity, sample_mask = gradient_per_image(width, depth1, period)
            if config['logging']:
                torch.save(emissivity, f"{round(width*1000)/1000}_{round(depth1*1000)/1000}_period{period}_10_long_cut_emission.pt")
            idx = sample_mask.nonzero(as_tuple=True)[0]
            fom = ff.power_ratio(wavelengths[idx], emissivity[idx], ff.T_e, .726)
            print(fom.detach())
            # if plotting:
