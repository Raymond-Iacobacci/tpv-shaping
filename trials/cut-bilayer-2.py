...
The bilayer system has a base of homogeneous Tungsten and a top of a patterned Tungsten grating, randomly generated at the start.
'''
...
config = {
    ...
    "gradient_scale": float(1e12),
    "n_incidence_harmonics": int(361),
    "n_grating_harmonics": int(361), # >= config['n_incidence_harmonics'], this tells you how many you need to set
...
y_pt = 0
n_x_measurement_pts = config['n_grating_elements'] # Measuring one x-point per square
L=1.1
z_buff = .15
x_measurement_space = np.linspace(0, L, 2*n_x_measurement_pts+1)[1::2] # Taking the center of the n_x_measurement_pts squares
z_space = np.linspace(0, .5 + 1.6 + .473 + 1, 10)
...
# grating[:]=0
# grating[x]=1
# grating[random.sample(range(20), 10)] = 1
grating[18:]=0
grating[:18]=1
grating = torch.nn.Parameter(grating)
n_epochs = 1000
opt = torch.optim.Adam([grating], lr = config['learning_rate']) # Temporary resuming code
sched = StepLR(opt, step_size = 50, gamma = 0.90)

for epoch in range(n_epochs):
    opt.zero_grad()
    dfom_deps = torch.zeros((config['n_grating_elements']))
    dflux_deps = torch.zeros((wavelengths.shape[0], config['n_grating_elements'])) # There is a bug here where we skip over the elements in exclude_wavelengths. We assume that this bug's contribution is not significant when compared to the contribution from other approximations throughout this calculation. By initializing all these values to 0, the wavelengths that we skip over in the following calculation are not considered.
    ...
    for i_wl, wl in enumerate(tqdm(wavelengths, desc="Processing wavelengths", leave = False)):
        ...
        S = S4.New(Lattice = ((L, 0), (0, L)), NumBasis = config['n_grating_harmonics'])
        ...
        S.AddLayer(Name = 'Grating', Thickness = .473, Material = 'AlN')
        ...
        # for nx in range(2):
            ...
            # S.SetRegionRectangle(Layer = 'Grating', Material = f'Vac', Center = ((nx+1)/grating.shape[-1] - 1/(2*grating.shape[-1]), (ny+1)/grating.shape[-1] - 1/(2*grating.shape[-1])), Halfwidths = (1/(2*grating.shape[-1]), 1/(2*grating.shape[-1])), Angle = 0)
            # S.SetRegionRectangle(Layer = 'Grating', Material = f'Vac', Center = (((nx+1)/grating.shape[-1] - 1/(2*grating.shape[-1]))*L, .5*L), Halfwidths = (1/(2*grating.shape[-1])*L, .5*L), Angle = 0)
        # for nx in range(18):
            # for ny in range(18):
            # S.SetRegionRectangle(Layer = 'Grating', Material = f'AlN', Center = (((nx+1)/grating.shape[-1] - 1/(2*grating.shape[-1]))*L, .5*L), Halfwidths = (1/(2*grating.shape[-1])*L, .5*L), Angle = 0)
        S.SetRegionRectangle(Layer = 'Grating', Material = 'Vac', Center = (.5*L, .5*L), Halfwidths = (.1, .5*L), Angle = 0)
        S.SetRegionRectangle(Layer = 'Grating', Material = 'Vac', Center = ((1.1+.65)/2, .5*L), Halfwidths = ((1.1-.65)/2, .1), Angle = 0)
        S.SetRegionRectangle(Layer = 'Grating', Material = 'Vac', Center = ((.65)/2, .5*L), Halfwidths = ((1.1-.65)/2, .1), Angle = 0)
        ...
        fourier_adj_excitation = ff.create_step_excitation(basis = S_adj.GetBasisSet(), step_values = adj_excitation, num_harmonics = config['n_incidence_harmonics'], x_shift = 0, initial_phase = 0, amplitude = 1.0, plot_fourier = False) # Need to confirm that the polarization is indeed 'x'. I think that it might be 'y'. NOTE: I've set it to 'y' in the ff.py script-file.
        ...
        dflux_deps_wl = 2 * wl ** -2 * ff.e_0 * torch.real(torch.einsum('ijkl,ijkl->ijk', torch.as_tensor(grating_fields), torch.as_tensor(adj_grating_fields).conj())) # NOTE: fix from last time where we squared the wavelength. We are calculating frequency, not wavelength.
        dflux_deps[i_wl] = torch.mean(dflux_deps_wl, dim = 0).squeeze()
        del S_adj
    ...
    interpolated_data = interpolate_dataset(data, extend = 60, poly_order = 1) # Need to change to use torch.nn.functional.interpolate
    ...
    dfom_deps = torch.mean(dfom_dflux.unsqueeze(1).expand(wavelengths.shape[0], config['n_grating_elements']) * dflux_deps, dim = 0)
